...
class ModSet(Cog):
    ...
    async def message_tags(self, ctx, switch: Optional[str]):
        ...
        embed = Embed(title="Status", colour=0x00FF00, timestamp=datetime.utcnow())
    ...
    @command(
        name="goodbyeForever!",
        aliases=["leaveThisServer"],
        help="makes the bot to leave the server (only for server owner)",
    )
    async def leave_this_server(self, ctx):
        if ctx.author != ctx.guild.owner:
            embed = Embed(title="Status", colour=0xFF0000, timestamp=datetime.utcnow())
            embed.add_field(name="Error", value="Only server owner can use this command!", inline=True)
            await ctx.send(embed=embed)
            return
        else:
            with ctx.typing():
                ...
                embed.set_footer(
                    text="made by codechaser", icon_url="https://avatars.githubusercontent.com/u/63065397?v=4"
                )

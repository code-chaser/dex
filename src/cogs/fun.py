...
class Fun(commands.Cog):
    ...
    async def crypto_command(self, ctx, cryptocurrencies: str, *, currencies: Optional[str]):
        ...
        # API_URL = "https://api.coingecko.com/api/v3/coins/list"
        # data_json = aiohttp.ClientSession().get(API_URL)
        # data_json = await data_json.json(content_type=None)

        data_json = await self.get_all_crypto()
        cryptocurrencies = cryptocurrencies.split(",")
        for i in range(len(cryptocurrencies)):
            cryptocurrencies[i] = difflib.get_close_matches(cryptocurrencies[i].lower(), [element["name"].lower(
            ) for element in data_json] + [element["symbol"].lower() for element in data_json], 1, 0.8)[0]
            if cryptocurrencies[i] in [element["name"].lower() for element in data_json]:
                continue
            for j in data_json:
                if j["symbol"].lower() == cryptocurrencies[i]:
                    cryptocurrencies[i] = j["name"].lower()
                    break
